...
SHARD1_REPL_URI = os.getenv("SHARD1_REPL_URI")
SHARD2_REPL_URI = os.getenv("SHARD2_REPL_URI")
...
shard1_inspect = (
    motor.motor_asyncio.AsyncIOMotorClient(SHARD1_REPL_URI, serverSelectionTimeoutMS=10000)
    if SHARD1_REPL_URI
    else None
)
shard2_inspect = (
    motor.motor_asyncio.AsyncIOMotorClient(SHARD2_REPL_URI, serverSelectionTimeoutMS=10000)
    if SHARD2_REPL_URI
    else None
)
...
async def root():
    ...
    replica_member_counts: Optional[dict] = None
    if shard1_inspect and shard2_inspect:
        try:
            s1 = await shard1_inspect.admin.command("replSetGetStatus")
            s2 = await shard2_inspect.admin.command("replSetGetStatus")
            replica_member_counts = {
                "shard1": len(s1.get("members", [])),
                "shard2": len(s2.get("members", [])),
            }
        except (errors.PyMongoError, errors.OperationFailure) as ex:
            logger.debug("replica set inspect: %s", ex)
            replica_member_counts = None
    ...
    return {
        ...
        "shards": shards,
        "replica_member_counts": replica_member_counts,
